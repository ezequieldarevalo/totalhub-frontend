...
function UpcomingReservations({ initialReservations }) {
  ...
  const fetchReservations = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/backend/reservations/upcoming?${params.toString()}`, {
        headers: {
          Authorization: `Bearer ${document.cookie.split('token=')[1]}`,
        },
      });
  ...
  return (
    <DashboardLayout pageTitle="Próximas reservas">
      ...
      {reservations.length === 0 ? (
        <p>No hay próximas reservas registradas.</p>
      ) : (
...
export const getServerSideProps = withAuth(async (ctx) => {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reservations/upcoming`, {
      headers: {
