...
function DashboardReservations({ initialReservations }) {
  ...
  const fetchReservations = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/backend/reservations/dashboard?${params.toString()}`, {
        headers: {
          Authorization: `Bearer ${document.cookie.split('token=')[1]}`,
        },
      });
  ...
  return (
    <DashboardLayout pageTitle="Reservas">
      <h1>Reservas del hostel</h1>
        ...
        <table border="1" cellPadding="8" style={{ marginTop: '1rem', width: '100%' }}>
          ...
          <tbody>
            {reservations.map((r) => {
              ...
              const missingAmount = totalPrice - paidAmount;
...
export const getServerSideProps = withAuth(async (ctx) => {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reservations/dashboard`, {
      headers: {
